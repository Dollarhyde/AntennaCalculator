...
class PrintGenerator:
    ...
    def print_bowtie(self, filename, W, L, g):
        w_cm, h_cm = (22, 28)             # Size of letter paper in cm
        res_x, res_y = (300, 300)           # Desired resolution
        ...
        # center of bowtie
        originW = substrate_origin + W
        originL = substrate_origin + L

        #subtract gap and split to get triangle length
        wi = 0.5*W - 0.5*g

        # Draw bowtie
        points_right_cm = [(originW+0.5*g, originL), (originW+wi, originL+0.5*L), (originW+wi, originL-0.5*L), (originW+0.5*g, originL)]
        points_right_coords = [(int(c[0] / f * res_x), int(c[1] / f * res_y)) for c in points_right_cm]
        draw.polygon(tuple(points_right_coords), fill=(0, 0, 0))
       
        points_left_cm = [(originW-0.5*g, originL), (originW-wi, originL+0.5*L), (originW-wi, originL-0.5*L), (originW-0.5*g, originL)]
        points_left_coords = [(int(c[0] / f * res_x), int(c[1] / f * res_y)) for c in points_left_cm]
        draw.polygon(tuple(points_left_coords), fill=(0, 0, 0))
