...
class PrintGenerator:
    ...
    def print_patch(self, filename, W, L, x0, y0, Ws=None):
        ...
        if W * 2 > 22:
            print(
                "[*] The substrate is too large for letter paper. Please adjust paper size."
            )
        ...
        # Draw patch
        if self.args.type == "microstrip":
            microstrip_patch_coords_cm = [
                (originW, originL),
                (originW + W, originL),
                (originW + W, originL + L),
                ((originW + W_cut + Ws + g * 2), originL + L),
                (originW + W_cut + Ws + g * 2, originL + L - x0),
                (originW + W_cut + Ws + g, originL + L - x0),
                (originW + W_cut + Ws + g, originL + L * 1.5),
                (originW + W_cut + g, originL + L * 1.5),
                (originW + W_cut + g, originL + L - x0),
                (originW + W_cut, originL + L - x0),
                (originW + W_cut, originL + L),
                (originW, originL + L),
                (originW, originL),
            ]
            microstrip_patch_coords = [
                (int(c[0] / f * res_x), int(c[1] / f * res_y))
                for c in microstrip_patch_coords_cm
            ]
            draw.polygon(tuple(microstrip_patch_coords), fill=(0, 0, 0))
        elif self.args.type == "probe":
            probe_patch_coords_cm = [
                (originW, originL),
                (originW + W, originL),
                (originW + W, originL + L),
                (originW, originL + L),
            ...
            probe_patch_coords = [
                (int(c[0] / f * res_x), int(c[1] / f * res_y))
                for c in probe_patch_coords_cm
            ]
            draw.polygon(tuple(probe_patch_coords), fill=(0, 0, 0))

            probe_feed_coords_cm = [
                (originW + W - y0 - 0.0707 / 2, originL + L - x0 - 0.0707 / 2),
                (originW + W - y0 + 0.0707 / 2, originL + L - x0 + 0.0707 / 2),
            ]
            probe_feed_coords = [
                (int(c[0] / f * res_x), int(c[1] / f * res_y))
                for c in probe_feed_coords_cm
            ]
        ...
        substrate_coords_cm = [
            ...
            (substrate_origin + 2 * W, substrate_origin + 2 * L),
        ]
        substrate_coords = [
            (int(c[0] / f * res_x), int(c[1] / f * res_y)) for c in substrate_coords_cm
        ]
